...
export function useWishlist() {
    const { items, addToWishlist, removeFromWishlist, isInWishlist, setWishlist, clearWishlist, } = useWishlistStore();
    ...
    const { refetch: fetchWishlist } = useQuery({
        ...
        onError: (error) => {
            toast.error(error?.response?.data?.message || "Error al cargar la lista de deseos");
        },
    });
